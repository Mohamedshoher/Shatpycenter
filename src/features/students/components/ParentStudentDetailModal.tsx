...
export const ParentStudentDetailModal: React.FC<ParentStudentDetailModalProps> = ({
    ...
    const renderSchedule = () => {
        ...
        return (
            <div className="space-y-6">
                <div className="bg-indigo-50/50 p-4 sm:p-6 rounded-[24px] sm:rounded-[32px] border border-indigo-100 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                    <div className="flex items-center gap-3 min-w-0">
                        <div className="w-11 h-11 sm:w-14 sm:h-14 bg-indigo-600 rounded-2xl flex items-center justify-center text-white shadow-lg shadow-indigo-500/20 shrink-0">
                            <Clock size={24} />
                        </div>
                        <div className="min-w-0">
                            <h4 className="text-sm sm:text-lg font-black text-indigo-900 truncate">تحديد مواعيد الحضور</h4>
                            <p className="text-[10px] sm:text-xs text-indigo-600 font-bold">اختر الأيام والساعات المتاحة لابنك</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-1.5 bg-white px-3 py-1.5 rounded-full text-[10px] font-black text-indigo-600 border border-indigo-100 shadow-sm whitespace-nowrap">
                        <Users size={12} />
                ...
                <div className="space-y-3 sm:space-y-4">
                    {scheduleDays.map(day => {
                        ...
                        return (
                            <div key={day} className="bg-white p-4 sm:p-5 rounded-[24px] sm:rounded-[28px] border border-gray-100 shadow-sm">
                                <div className="flex items-center justify-between mb-3 sm:mb-4">
                                    <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
                                        <div className="w-9 h-9 sm:w-10 sm:h-10 bg-indigo-50 text-indigo-600 rounded-xl sm:rounded-2xl flex items-center justify-center font-black shrink-0">{day[0]}</div>
                                        <div className="min-w-0">
                                            <h5 className="text-sm font-black text-gray-900">{day}</h5>
                                            {savedTime ? (
                                                <p className="text-[10px] font-bold text-indigo-600 truncate">الموعد الحالي: {savedTime}</p>
                                            ) : (
                                    ...
                                    {savedTime && (
                                        <button
                                            ...
                                            className="w-9 h-9 flex items-center justify-center text-red-400 hover:bg-red-50 rounded-xl transition-colors shrink-0"
                                    ...
                                    {availableHours.map(hour => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                disabled={isFull}
                                                className={cn(
                                                    "relative rounded-xl sm:rounded-2xl border-2 px-1 py-2 sm:p-3 text-center transition-all duration-300 min-w-0",
                                                    isSelected
                                            ...
                                            >
                                                <p className="text-xs sm:text-sm font-black">{hour > 12 ? hour - 12 : hour}:00</p>
                                                <p className={cn("text-[8px] sm:text-[9px] font-bold mt-0.5 truncate", isSelected ? "text-indigo-100" : isFull ? "text-gray-300" : isCurrent ? "text-indigo-500" : "text-gray-400")}>
        ...
        <>
            ...
            <SlideIn show={isOpen} className="fixed inset-0 z-[101] flex items-center justify-center p-4">
                <div className="bg-gray-50 w-full max-w-5xl h-[90vh] rounded-[48px] overflow-hidden flex flex-col shadow-2xl border border-white/20"
                    ...
                    <div className="flex-1 overflow-y-auto p-4 sm:p-8 sm:pt-4 custom-scrollbar">
